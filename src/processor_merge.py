...
class DataMerger:
    ...
    def _deduplicate_updates(self, df_updates):
        window_spec = Window.partitionBy("stop_id").orderBy(col("update_timestamp").desc())
        return (
        ...
        return (
            df_merged.withColumn(
                ...
                when(
                    ...
                    when(
                        (unix_timestamp("actual_arrival") - unix_timestamp("planned_arrival"))
                        / 60
                        < 0,
                        0,
                    ).otherwise(
            ...
            .withColumn(
                ...
                when(
                    ...
                    | (col("departure_status").isNotNull() & (col("departure_status") == "c")),
                    True,
                ).otherwise(False),
            )
            .withColumn(
            ...
            .withColumn(
                "hour",
                when(col("planned_departure").isNotNull(), hour("planned_departure")).otherwise(
                    hour("planned_arrival")
                ),
            )
        )
