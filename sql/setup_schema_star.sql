...
CREATE INDEX idx_fact_planned_path ON fact_train_movement (planned_path) WHERE planned_path IS NOT NULL;

-- Task 4.2: Fast lookup by station and time for earliest arrival
CREATE INDEX idx_fact_station_time ON fact_train_movement (station_key, snapshot_time_key);
CREATE INDEX idx_fact_train_station ON fact_train_movement (train_key, station_key, planned_time_key);
